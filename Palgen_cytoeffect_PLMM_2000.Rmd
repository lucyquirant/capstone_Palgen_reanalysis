---
title: "Palgen cytoeffect PLMM 2000"
author: "Lucy Quirant"
date: "4/23/2019"
output: 
  html_document:
...
pars_str = c(
  ...
  "sigma[1]", "sigma[2]", "Cor[1,2]", "Cor[21,3]",  "sigma[10]",
  "sigma_term[1]", "sigma_term[2]", "Cor_term[1,2]", "Cor_term[15,12]", "Cor_term[10,1]", "sigma_term[10]",
  "sigma_donor[1]", "sigma_donor[2]", "Cor_donor[1,2]", "sigma_donor[10]"
  )
...
ggsave(filename = "posterior_mds_latent_variable_mu_scaled_2000.pdf", width = 10, height = 5)
cytoeffect::plot_mds(obj, asp = FALSE)
ggsave(filename = "posterior_mds_latent_variable_mu_unscaled_2000.pdf", width = 10, height = 5)
```
This plot further confirms what has been shown throughout this analysis. Post-prime and post-boost do not differ significantly. They are clearly overlapped. The proteins are all clustered and not easy to read. The MDS1 and MDS2 only account for a small amount of variance.
...
Interesting to see which proteins correlate more strongly with each other. 
...
ggsave(plot = pall, 
       filename = "posterior_summary_plmm_2000.pdf", 
       width = 8, height = 11)
```
...
#save
save(gof_obsv_all, file = "PLMM_gof_obs_2000.Rdata")
save(gof_pred_all, file = "PLMM_gof_pred_2000.Rdata")
```
