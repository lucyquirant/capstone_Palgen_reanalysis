...
##Download data
The data can be accessed with the package `FlowRepositoryR` to download `fcs` files from [FlowRepository](http://flowrepository.org/id/FR-FCM-ZYPY). Only downloaded files 6 hours after prime vaccination (PPD000H06) and 6 hours after boost vaccination (PBD000H06).
...
ggplot(df_samples_all, aes_string(x = "TNFa", color = "term")) + 
  ...
  facet_wrap(~donor)
```
From the plots about IFNg, it can be seen that in most animals, IFNg is higher after the boost vaccination has been administered.
...
ggplot(df_samples_all, aes_string(x = "CD66", y = "CD56")) + 
  ...
  facet_wrap(~donor)
```
CD3 - CD8 plot is not confirming the gating as they should be CD3 negative and CD8 positive. There seems to be a wide variety.
...
The overlap between the two results is relatively similar, both found the two following markers: CD66 and CD107a. For all other proteins, the other method did not find the same result. This high discrepancy may be due to a number of reasons.
- Sample size is very small and there is very high between donor variation
- Different statistical methods were applied (with different regularization)
  - PERSONAL: Look more into stats!
- Almost all proteins do not seem to differ strongly between the two conditions and have rather large standard deviations (PRODUCE GRAPH? look at SD and Boost - Prime graph with zero line - in PLMM - can do equivalent?)
- Another explanation for the difference is that I am only using a subset of the data. For simplicity sake to not incorporate the different time points.
