...
ggplot(df_samples_tfm2, aes(numeric_term, CD66)) + 
   geom_smooth(method = mgcv::gam, formula = CD66 ~ s(numeric_term), data = df_samples_tfm2) +
  theme(axis.text.x = element_text(angle = 45, vjust=0))

#add median column
#facet on condition - is grouped_term
#creates phases - include 0 hours of the condition - phase 1 prime & phase 2 boost -> can facet_wrap
```

```{r}
#plots successfully the median markers in base R
plot(df_median_fct2$numeric_term, df_median_fct2$CD66, type = "p", col=df_median_fct2$grouped_term)

#plots successfully the spline regression - y axis different
model <- gam(CD66 ~ s(numeric_term), data = df_samples_tfm2)
plot(model,
...
#try to combine two plots - WORKS ! y axis overwritten, not same scale, unreadable y label name - points look pretty close to the spline regression
plot(df_median_fct2$numeric_term, df_median_fct2$CD66, type = "p", col=df_median_fct2$grouped_term)
par(new=TRUE)
plot(model)

unique(df_median_fct2$grouped_term)
...
#tried to put under first plot, doesn't work - doesn't plot - no error given
legend(x=14, y=3,legend=paste(c("Before-prime", "Post-prime", "Post-boost")),pch=rep(16,each=3),bty="n",cex=0.7,pt.cex=0.7)
#doesn't work either
legend(x=14, y=3, legend=paste(c("Before-prime", "Post-prime", "Post-boost")), pch = "sSvV", col = c(1, 3),
       cex = 1+(-1:2)/8)
#suggested to use points() or lines()
#lines(gam(CD66 ~ s(numeric_term), data = df_samples_tfm2))
```
```{r}
...
In spline regression, PPD0H06 = 4 and PBD0H06 = 10
